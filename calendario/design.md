...
│   │   └── supabase.ts       # Client anon do Supabase (opcional no front)
│   ├── hooks/
...
│       ├── id.jpeg           # Variante do background
...
- **Eventos publicos**: `GET /api/events` (anon, sem auth)
- **CRUD admin**: `POST/PATCH/DELETE /api/admin/events` (requer cookie JWT)
- **Auth**: `POST /api/auth/login`, `/logout`, `/me`, `/setup`
...
| Metodo | Rota                       | Descricao              |
|--------|----------------------------|------------------------|
| POST   | `/api/auth/setup`          | Cria 1o admin (X-Setup-Token) |
| POST   | `/api/auth/login`          | Login → cookie JWT     |
| POST   | `/api/auth/logout`         | Limpa cookie           |
| GET    | `/api/auth/me`             | perfil do admin logado |
| POST   | `/api/admin/events`        | Cria evento            |
| PATCH  | `/api/admin/events/:id`    | Atualiza evento        |
...
- `SETUP_TOKEN` usado uma unica vez para criar o primeiro admin
