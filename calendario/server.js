...
// ── Auth (consolidado: GET/POST /api/auth?op=...) ──────────────
...
// ── Events CRUD (consolidado: GET/POST/PATCH/DELETE /api/events) ──
...
app.patch('/api/event-types', async (req, res) => {
  ...
  try {
    ...
    if (!id) return res.status(400).json({ error: 'id_required' });
    if (fields.key) {
      fields.key = String(fields.key).toLowerCase().trim().replace(/[^a-z0-9_-]/g, '-');
    }
    const { data, error } = await supabaseAdmin
      .from('event_types')
      .update(fields)
...
// ── Auth (consolidado: /api/auth?op=login|logout|me|setup) ────
