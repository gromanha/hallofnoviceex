...
const IMPORT_RATE_WINDOW = 60 * 60 * 1000; // 1 hour
const IMPORT_TIMEOUT = 30_000;
const importRateMap = new Map();

function checkImportRate(ip) {
  const now = Date.now();
  const entry = importRateMap.get(ip);
  if (!entry || now - entry.start > IMPORT_RATE_WINDOW) {
    importRateMap.set(ip, { start: now, count: 1 });
    return true;
  }
  entry.count++;
  return entry.count <= IMPORT_RATE_LIMIT;
}

// Category mapping from wiki URL patterns
const WIKI_CATEGORY_MAP = {
  phantom_weapons: 'guias',
  relics: 'guias',
  quests: 'noticias',
  items: 'receitas',
  crafting: 'receitas',
...
// Import rate limiter for wiki imports
...
app.post('/api/posts/import', importRateLimit, async (req, res) => {
  const claims = requireAdmin(req, res);
  if (!claims) return;
    ...
    if (!rawContent.trim()) {
      try {
        ...
        if (page && page.missing === undefined) {
          ...
          if (wikitext) {
            // Basic wikitext cleanup for server fallback
            rawContent = wikitext
